...
module BestFirst =
    ...
    let Solve rn = 
        ...
        let IsSmartState (s:State) l = 
            match s with
            | S(_,sm,tm,rm,ps) -> // Check if any train is near any of the changed location, if not state is not relevant
        ...
        let IsSafeState (s:State) =
            match s with
            | S(_,sm,tm,rm,ps) -> // Checks if trains can reach other or can go off path, if true state is not relevant
        ...
        let rec ControllerTurn _ =
            ...
            | _ -> let (s,p) = PriorityQueue.pop unexploredStatesController
                   ...
                   | _ ->      match s with
                               | S(x,sm,tm,rm,_) ->  // Shut of all signals, possibly find smarter way to do
                                                     let sm = Map.fold (fun s k v -> Map.add k false s) Map.empty sm
                                                     // Find the highest priority of train not yet in goal
